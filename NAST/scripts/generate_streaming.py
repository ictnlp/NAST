...
def _main(cfg: DictConfig, output_file):
    ...
    for sample in progress:
        ...
        gen_timer.start()
        hypos, actions = task.inference_step(
            generator,
            models,
            sample,
        ...
        for i, sample_id in enumerate(sample["id"].tolist()):
            ...
            j=0
            _hypo_tokens = hypos[i]     
            action_seq = " ".join(actions[i])
            ...
            if not cfg.common_eval.quiet:
                ...
                # READ-WRITE sequence
                print(
                    "RW-{}\t{}\t{}".format(sample_id, np.around(RW2AL(action_seq), 2), action_seq),
                    file=output_file,
                )        
    ...
    cw, ap, al, dal = compute_delay(action_seqs, is_weight_ave=True)
